...
bool CheckInputEvent(void)
{
	fd_set	rfd;
	struct	timeval to;
	struct	input_event	ts_data, kpd_data;
	int	max_fd, ret;
	bool	event;
	...
	if (ret < 0) {
		...
		// check touchscreen input event
		/*if (FD_ISSET(fd_ts, &rfd)) {
			read(fd_ts, &ts_data, sizeof(ts_data));
			if ((ts_data.code == ABS_PRESSURE) && (ts_data.value > 0)) {
				//printf("ts: %d\n", ts_data.value);
				event = true;
			}
		}*/
