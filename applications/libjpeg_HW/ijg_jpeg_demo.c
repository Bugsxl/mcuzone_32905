...
void decompress( char * jpeg_filename, int enableScaleDown )
{
	...
	if ( enableScaleDown )
	{
		...
		unsigned short u16RealWidth, u16RealHeight, u16YUVFormat;
...
//	cinfo.do_block_smoothing = TRUE;			// don't care, hw not support
	...
	memset( g_sDemo_Lcm.pLCMBuffer, 0, (g_sDemo_Lcm.fb_finfo.line_length * g_sDemo_Lcm.fb_vinfo.yres) );
	...
	{
		memcpy( g_sDemo_Lcm.pLCMBuffer, _frame_buffer, display_width );
		g_sDemo_Lcm.pLCMBuffer += (g_sDemo_Lcm.fb_vinfo.xres * g_sDemo_Lcm.u8bytes_per_pixel);
