...
void setup_time(int fd){
	...
	scanf("%s %s %s %s %s %s", year, month,day,hour,min,sec);
...
/*
*setup_wkalarm():set up the sec,func
*/	
void setup_wkalarm(int fd,int sec,void(*func)()){
	...
	rtc_alarmtm.enabled=1;
	rtc_alarmtm.time=rtc_tm;

	/* Set the alarm to 5 sec in the future */

	retval = ioctl(fd, RTC_WKALM_SET, &rtc_alarmtm);
	if (retval <0) {
	...
	retval = ioctl(fd, RTC_WKALM_RD, &rtc_alarmtm);
	if (retval <0) {
...
/*
*setup_tick():set up the tick,func
*/
void setup_tick(int fd ,unsigned int tick,void(*func)())
{
	int ret,tcount=0,icount=10;
	unsigned long data;
  
	ret = ioctl(fd,RTC_TICK_ON,0);
	if (ret <0) {
		printf("!!!RTC_TICK_ON faild!!!\n");
		return;
	}
	ret = ioctl(fd,RTC_TICK_SET,&tick);
	if (ret <0) {
		printf("!!!RTC_TICK_SET faild!!!\n");
		return;
	}
	ret =ioctl(fd,RTC_TICK_READ,&tcount);
	if (ret <0) {
		printf("!!!RTC_TICK_SET faild!!!\n");
		return;
	}else{
	...
	/* This blocks until the tick ring causes an interrupt */
	while(icount){
	...
	if (ret == -1) {
		printf("ioctl RTC_TICK_OFF faild!!!\n");
		return ;
	}
	printf("Test periodic tick test OK!\n");	
}
...
int main(void) {
	...
	printf("\n******** RTC Test Demo ***********\n");
      	...
      	printf("4.Test second alarm(hour,min,sec)..\n");
      	printf("5.Test periodic tick ..\n");
