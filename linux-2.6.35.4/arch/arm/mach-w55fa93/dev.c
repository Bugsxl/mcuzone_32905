...
static struct platform_device w55_gpio_leds_device = {
	.name			= "leds-gpio",
	.id			= -1,
	.dev.platform_data	= &led_data,
};

void __init w55_gpio_leds(struct gpio_led *leds,int nr)
{
	//int i;
	...
	//for (i = 0; i < nr; i++)
		...
		w55fa93_gpio_set(GPIO_GROUP_B,3,leds[0].active_low) ;
		w55fa93_gpio_set(GPIO_GROUP_B,4,leds[1].active_low) ;
		w55fa93_gpio_set(GPIO_GROUP_B,5,leds[2].active_low) ;
		w55fa93_gpio_set(GPIO_GROUP_B,6,leds[3].active_low) ;

	led_data.leds = leds;
	led_data.num_leds = nr;
	platform_device_register(&w55_gpio_leds_device);
}
...
void __init w55fa93_dev_init()
{
	...
	w55_gpio_leds(ek_leds,ARRAY_SIZE(ek_leds));
