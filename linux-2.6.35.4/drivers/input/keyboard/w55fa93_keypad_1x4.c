...
static void read_key(unsigned long arg)
{
        ...
        new_key = readl(REG_GPIOA_PIN) & 0x40;

        if ((new_key & 0x40) == 0x40) { // all key released
//	  printk("->0\n");
	...
	new_key = (~(readl(REG_GPIOA_PIN)) & 0x40) >> 2;
...
static irqreturn_t w55fa93_kpd_irq(int irq, void *dev_id) {
        ...
        // clear source
        writel(src & 0x00000040, REG_IRQTGSRC0);
        return IRQ_HANDLED;
}
...
int w55fa93_kpd_open(struct input_dev *dev) {
        ...
        writel(readl(REG_IRQTGSRC0) & 0x0000040, REG_IRQTGSRC0); // clear source
	...
	writel((readl(REG_IRQENGPA)& ~(0x00400000)) | 0x0000040, REG_IRQENGPA); 
...
void w55fa93_kpd_close(struct input_dev *dev) {
		...
		writel((readl(REG_IRQENGPA)& ~(0x00400040)), REG_IRQENGPA); 
                del_timer(&kpd_timer);
...
static int __init w55fa93_kpd_reg(void) {
        ...
        // init GPIO
        // PORTA[2-5]
        writel(readl(REG_GPIOA_OMD) & ~((1 << 6) | (1 << 6) | (1 << 6) | (1 << 6)), REG_GPIOA_OMD); // input
        writel(readl(REG_GPIOA_PUEN) | ((1 << 6) | (1 << 6) | (1 << 6) | (1 << 6)), REG_GPIOA_PUEN); // pull-up
        writel(readl(REG_IRQSRCGPA) & ~(0xFC00), REG_IRQSRCGPA); // GPA[2~5] as nIRQ0 source
	writel((readl(REG_IRQENGPA)& ~(0x00400000)) | 0x00000040, REG_IRQENGPA); // falling edge trigger
