    ...
    if (!cmd->error) {
        if ((err & SDISR_CRC_7) == 0) {
            if (!(mmc_resp_type(cmd) & MMC_RSP_CRC)) {
                ...
                cmd->error = -EIO;
                nvt_sd_debug("Error detected and set to %d/%d (cmd = %d, retries = %d)\n",
...
static int nvt_sd_get_ro(struct mmc_host *mmc)
{
    ...
    if (host->port == 0)
    {
        ...
        {
            //nvt_sd_debug("nvt_sd_get_ro(): SD port %d is write unprotected.\n", host->port);
            return 0;   // write unprotected
        }
        else
        {
            //nvt_sd_debug("nvt_sd_get_ro(): SD port %d is write protected.\n", host->port);
