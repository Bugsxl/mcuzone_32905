...
void I2C_Delay(UINT32 u32Delay)
{
	...
	{/* GPB02 reset:	H->L->H 	*/		
		...
		udelay(100);
		w55fa93_gpio_set(GPIO_GROUP_B, 2, 0);	//GPIOB2 set low
		udelay(100);		
		w55fa93_gpio_set(GPIO_GROUP_B, 2, 1);	//GPIOB2 set high
		return 0;
	}
	#endif
	...
	{/* GPB3 power down, HIGH for power down */
		...
		if(bIsEnable)
			w55fa93_gpio_set(GPIO_GROUP_B, 3, 1);		//GPIOB3 set high
		else			
			w55fa93_gpio_set(GPIO_GROUP_B, 3, 0);	//GPIOB3 set low	
		return 0;
	}
	#endif
#endif
	...
	static BOOL SnrReset(void)
	{/* GPB04 reset:	H->L->H 	*/	
		outp32(REG_GPBFUN, inp32(REG_GPBFUN0) & (~MF_GPB4));
		w55fa93_gpio_configure(GPIO_GROUP_B, 4);
		w55fa93_gpio_set(GPIO_GROUP_B, 4, 1);
		w55fa93_gpio_set_output(GPIO_GROUP_B, 4);
		schedule_mdelay(10);
		w55fa93_gpio_set(GPIO_GROUP_E, 4, 0);	//GPIOB 4 set low
		schedule_mdelay(10);	
		w55fa93_gpio_set(GPIO_GROUP_E, 4, 1);	//GPIOB 4 set high
		return 0;
	}
	#endif
	#if defined(CONFIG_SENSOR_POWER_DOWN)
	static BOOL SnrPowerDown(BOOL bIsEnable)
	{/* GPB5 power down, HIGH for power down */
		outp32(REG_GPBFUN, inp32(REG_GPBFUN0) & (~MF_GPB5));
		//while(1);
		w55fa93_gpio_configure(GPIO_GROUP_B, 5);
		w55fa93_gpio_set(GPIO_GROUP_B, 5, 1);
		w55fa93_gpio_set_output(GPIO_GROUP_B, 5);
		if(bIsEnable)
			w55fa93_gpio_set(GPIO_GROUP_B, 5, 1);	//GPIOB 5 set high
		else			
			w55fa93_gpio_set(GPIO_GROUP_B, 5, 0);	//GPIOB 5 set low	
		return 0;
	}
	#endif
#endif
