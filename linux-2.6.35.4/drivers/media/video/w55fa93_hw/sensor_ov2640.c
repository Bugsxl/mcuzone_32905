...
#endif
#ifdef CONFIG_SENSOR_OV2640_DEV1
static struct OV_RegValue g_sOV2640_RegValue[] = 
{
	//NewKen sensor module initial for OV2640 20110613
 /* 640x480 */
    {0xff, 0x01},{0x12, 0x80},
...
#elif defined  CONFIG_SENSOR_OV2640_DEV1
	{g_sOV2640_RegValue,_REG_TABLE_SIZE(g_sOV2640_RegValue)},
#else
	{0,0}
#endif
};
...
static __s32 InitSensor(__u32 u32Sensor, void *priv)
{
	...
	{//OV9660 will be 30f/s @ sensor clock 24MHz for VGA output.  
		case OV_2640:
			...
			vin_priv->sensor_intf->u8SensorDevID = g_uOvDeviceID[0];//g_uOvDeviceID[pDevVin->ov2640];
		...
		/* Cliff's demo board has not sensor reset and power down control */
			...
			if( res<0 ){
				...
				printk("Sensor initial successful \n");
			vin_priv->sensor_intf->u8SensorDevID = g_uOvDeviceID[0];//g_uOvDeviceID[pDevVin->ov7670];
			vin_priv->sensor_intf->u16CurImgHeight = vin_priv->sensor_intf->u16MaxImgHeight;
			vin_priv->sensor_intf->u16CurImgWidth = vin_priv->sensor_intf->u16MaxImgWidth;
