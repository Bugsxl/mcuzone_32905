...
/* linux/driver/scsi/nuvoton_gnand/w55fa93_nand.c
*
...
//#define FMI_SM_INIT_ERROR    -1
#define Fail                 -2
#define Get_block_addr_Fail  -3
//#define FMI_SM_RB_ERR        -4
#define Get_block_data_err    -5
//#define FMI_SM_ECC_ERROR      -6
//#define FMI_SM_STATE_ERROR    -7
//#define FMI_SM_STATUS_ERR     -8
...
static void nand_host_reset(void)
{
        ...
        // enable SM
        //outpw(REG_FMICSR, 0x08);  // ya..

//	 outpw(REG_SMTCR, 0x10305);     //  AHB:cpu=1:1
...
#if 0
                printk("GNAND_read(lba=0x%4x, count=0x%4x, buffer=0x%4x\n", lba, count, (u32)_fmi_pNANDBuffer1);
                printk("read buffer = \n");
                for(ii=0; ii<count*512; ii+=0x10)
                {
					for(jj=0; jj<0x10; jj++)
					{
                		printk("0x%2x ", _fmi_pNANDBuffer1[ii*0x10 +jj]);
					}
 	              		printk("\n");
				}
#endif
