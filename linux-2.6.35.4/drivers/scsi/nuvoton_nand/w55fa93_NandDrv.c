...
/* linux/driver/scsi/nuvoton_gnand/w55fa93_NandDrv.c
 *
 ...
 *   2011/03/04     CJChen1@nuvoton.com, to support NAND ROM chip "Infinite IM90A001GT"
 ...
 *                     for ROM chip always.
 *   2011/03/22     CJChen1@nuvoton.com, to support NAND ROM chip "Infinite IM90A001GT" on both MTD and RS.
...
//      printk("wait NAND data ready in READ_512 !\n");

//      printk("regiater SM_ISR = %4x \n ", inpw(REG_SMISR));
        while (!_fmi_bIsSMDataReady);
//      printk("regiater SM_ISR = %4x \n ", inpw(REG_SMISR));
//        printk("NAND data ready in READ_512 !\n");
...
//        printk("NAND data copy BEGIN !\n");
        memcpy((char *)uDAddr, (char *)_fmi_pNANDBuffer, 512);
//        printk("NAND data copy END !\n");
...
int fmiSM_Read_RA(FMI_SM_INFO_T *pSM, u32 uPage, u32 ucColAddr)
{
        ...
        outpw(REG_SMCMD, 0x00);     // read command
        outpw(REG_SMADDR, ucColAddr);   // CA0 - CA7
//        outpw(REG_SMADDR, (ucColAddr >> 8) & 0x1f);   // CA8 - CA11
...
//printk("sicSMChangeBadBlockMark\n");
...
#if 1
    // FA931 doesn't write "550091" to block-0
