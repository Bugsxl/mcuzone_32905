...
//	sharp_register_set(0x12, 0x0060);    // Entry mode  // DEN mode
	...
	//	w55fa93_upll_clock = 192000;
		clock_div = w55fa93_upll_clock / 27000;//5000;
		clock_div /= 2;
		clock_div &= 0xFF;
		clock_div --;
	...
	//   	outl(inl(REG_GPCFUN) | 0xFFFFFFFF, REG_GPCFUN);						// enable LVDATA[15:0] pins
		outl(inl(REG_GPCFUN) | 0x0000FFFF, REG_GPCFUN);
	...
	//   	outl(inl(REG_LCM_LCDCPrm)& ~LCDCPrm_LCDTYPE, REG_LCM_LCDCPrm);	// High Resolution mode
		outl((inl(REG_LCM_LCDCPrm)& ~LCDCPrm_LCDTYPE) | 0x01, REG_LCM_LCDCPrm);	
		/*
	...
	//   	outl( (inl(REG_LCM_LCDCCtl)& ~LCDCCtl_PRDB_SEL) | (0x00 << 20), REG_LCM_LCDCCtl);	// 16-bit mode 
		...
		outl( (inl(REG_LCM_LCDCCtl)& ~LCDCCtl_FBDS) | (0x1 << 1), REG_LCM_LCDCCtl);

		//outl((inl(REG_LCM_LCDCPrm)& ~LCDCPrm_LCDDataSel) | 0x0C, REG_LCM_LCDCPrm);
		outl( (inl(REG_LCM_LCDCPrm)& ~LCDCPrm_LCDDataSel) | (0x03 << 2), REG_LCM_LCDCPrm);
		/*
		0x0  // 16-bit mode (RGB565 output)
		0x1  // 18-bit mode (RGB666 output)
		0x2  // 24-bit mode (RGB888 output)		
		*/
		outl( (inl(REG_LCM_LCDCPrm)& ~LCDCPrm_SRGB_EL_SEL) | (0x02 << 6), REG_LCM_LCDCPrm);
		outl( (inl(REG_LCM_LCDCPrm)& ~LCDCPrm_SRGB_OL_SEL) | (0x02 << 4), REG_LCM_LCDCPrm);
	...
	//	outl((inl(REG_LCM_TCON1)& 0xFF000000)|(0x00035555), REG_LCM_TCON1);
		...
		// set both "active pixel per line" and "active lines per screen" for Syn type LCD   
	   	//outl(0x014000F0, REG_LCM_TCON3);
		outl(0x03BF00F0, REG_LCM_TCON3);	// 320x240 (0x140*0x0F0)
	   	
	   	outl(0x0140000E, REG_LCM_TCON4);
